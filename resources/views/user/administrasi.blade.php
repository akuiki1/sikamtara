<x-layout>
    ...
    <section class="px-10 py-10 bg-gray-50">
        {{-- logika table --}}
        <div x-data="{
            search: '',
            filterRole: '',
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 rounded-xl">
                    ...
                    <template x-if="filteredAdministrasi.length === 0">
                        <div class="col-span-full text-center text-gray-500 py-6">
                            Layanan Administrasi tidak ditemukan.
                            {{-- silahkan hubungi admin (button hubungi admin) --}}
                        </div>
                    </template>
            ...
            <div x-data="{ showSuccess: {{ session('success') ? 'true' : 'false' }}, showError: {{ session('error') ? 'true' : 'false' }} }" x-init="setTimeout(() => {
                showSuccess = false;
                showError = false
            }, 5000)" class="fixed top-5 right-5 z-50 space-y-2">
