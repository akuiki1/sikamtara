<x-layout>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 rounded-xl">
                ...
                <template x-if="filteredAdministrasi.length === 0">
                    <div class="col-span-full text-center text-gray-500 py-6 space-y-4" x-data="{ modalOpen: false }">
                        <p>Layanan Administrasi tidak ditemukan.</p>
    ...
    <section class="px-10 pt-4 pb-10 bg-gray-50" x-data="{
        ...
        get filteredAdministrasi() {
            return this.pengajuanAdministrasi.filter(item => {
                const matchesSearch = `${item.id_pengajuan_administrasi}`.toLowerCase().includes(this.search.toLowerCase());
                const matchesRole = this.filterRole === '' || item.role === this.filterRole;
                const matchesStatus = this.filterStatus === '' || item.status_verifikasi === this.filterStatus;
                return matchesSearch && matchesRole && matchesStatus;
            });
        ...
        <div class="md:col-span-4 bg-white p-5 rounded-2xl shadow">
            ...
            <div class="flex flex-col md:flex-row items-center justify-between gap-4 mb-4">
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    @php
                        $filterAktif = collect([
                            'search.riwayat' => request('search.riwayat'),
                            'role' => request('role'),
