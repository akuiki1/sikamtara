<x-layout>
    ...
    <section class="p-6 space-y-4 bg-gray-100">
        ...
        <div
            ...
            {{-- CTA Pilih Tahun --}}
            @php
                $tahunList = \App\Models\TahunAnggaran::orderByDesc('tahun')->get();
            @endphp
            ...
            @if ($tahunList->count())
                <form method="GET" action="{{ route('user.keuangan') }}" class="mt-4 md:mt-0">
                    ...
                    <select name="tahun" id="tahun" onchange="this.form.submit()"
                        ...
                        @php
                            $selectedTahun = $tahun->id_tahun_anggaran ?? request('tahun');
                        @endphp
                        @foreach ($tahunList as $th)
                            <option value="{{ $th->id_tahun_anggaran }}"
                                {{ $th->id_tahun_anggaran == $selectedTahun ? 'selected' : '' }}>
                                {{ $th->tahun }}
                            </option>
                        @endforeach
                    </select>
            ...
            @else
                <div class="text-sm text-gray-500 mt-4">Belum ada data tahun anggaran tersedia.</div>
            @endif
        </div>
        ...
        @if ($tahun)
            <div
                class="rounded-xl shadow bg-white p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">

                <div
                    class="flex-1 bg-green-100 p-3 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200">
                ...
                <div class="flex-1 bg-red-100 p-3 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200">
                    ...
                    <p class="text-xl font-bold text-red-900 mt-1">Rp {{ number_format($totalBelanja, 2, ',', '.') }}
                    </p>
                </div>

                <div
                    class="flex-1 bg-indigo-100 p-3 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200">
                    <p class="text-xs text-indigo-800 font-semibold">Surplus/Defisit</p>
                    <p class="text-xl font-bold text-indigo-900 mt-1">Rp
                        {{ number_format($surplusDefisit, 2, ',', '.') }}</p>
                </div>

                <div
                    class="flex-1 bg-purple-100 p-3 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200">
                    <p class="text-xs text-purple-800 font-semibold">Pembiayaan Netto</p>
                    <p class="text-xl font-bold text-purple-900 mt-1">Rp
                        {{ number_format($pembiayaanNetto, 2, ',', '.') }}</p>
                </div>

                <div
                    class="flex-1 bg-yellow-100 p-3 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200">
                    <p class="text-xs text-yellow-800 font-semibold">SILPA {{ $tahun->tahun }}</p>
                    <p class="text-xl font-bold text-yellow-900 mt-1">Rp {{ number_format($silpa_akhir, 2, ',', '.') }}
                    </p>
                </div>

            </div>
        @else
            <div class="p-4 bg-white rounded-xl shadow text-center text-gray-500">
                Belum ada data tahun anggaran yang tersedia.
            </div>
        @endif
                                        ...
                                        <x-slot name="body">
                                            @foreach ($b->rincianBelanja as $r)
                                                <tr>
                                                    ...
                                                    <td class="px-4 py-2 text-right">Rp
                                                        {{ number_format($r->anggaran - $r->realisasi, 2, ',', '.') }}
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </x-slot>
