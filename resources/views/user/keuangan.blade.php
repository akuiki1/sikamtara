        ...
        <div>
            ...
            <div class="bg-white rounded-xl p-4 mb-4 shadow-lg">
                ...
                <x-table>
                    <x-slot name="head">
                        <tr>
                            <th class="p-2 text-left w-1/2">PENDAPATAN DESA KAMBAT UTARA</th>
                            <th class="p-2 text-right w-1/6">Anggaran</th>
                            <th class="p-2 text-right w-1/6">Realisasi</th>
                            <th class="p-2 text-right w-1/6">Selisih</th>
                        </tr>
                    </x-slot>
                    <x-slot name="body">
                        @foreach ($pendapatan as $item)
                            <tr>
                                <td class="p-2 w-1/2">{{ $item->nama }}</td>
                                <td class="p-2 text-right w-1/6">Rp {{ number_format($item->anggaran, 2, ',', '.') }}
                                </td>
                                <td class="p-2 text-right w-1/6">Rp {{ number_format($item->realisasi, 2, ',', '.') }}
                                </td>
                                <td class="p-2 text-right w-1/6">Rp
                    ...
                    <x-slot name="footer">
                        ...
                        <tr class="font-semibold border-t">
                            <td class="p-2 text-left">Total Pendapatan Desa Kambat Utara Tahun {{ $tahun->tahun }}</td>
                            <td class="p-2 text-right">Rp {{ number_format($totalAnggaran, 2, ',', '.') }}</td>
                            <td class="p-2 text-right">Rp {{ number_format($totalRealisasi, 2, ',', '.') }}</td>
                            <td class="p-2 text-right">Rp {{ number_format($totalSelisih, 2, ',', '.') }}</td>
                        </tr>
                    </x-slot>
                </x-table>
            </div>
                    ...
                    @foreach ($belanja as $b)
                        @if ($b->rincianBelanja->count() > 0)
                            <x-table>
                                <x-slot name="head">
                                    <tr>
                                        <th class="p-2 text-left w-1/2">Belanja {{ $b->nama }}</th>
                                        <th class="p-2 text-right w-1/6">Anggaran</th>
                                        <th class="p-2 text-right w-1/6">Realisasi</th>
                                        <th class="p-2 text-right w-1/6">Selisih</th>
                                    </tr>
                                ...
                                <x-slot name="body">
                                    @foreach ($b->rincianBelanja as $r)
                                        <tr class="border-b">
                                            <td class="p-2 w-1/2">{{ $r->nama }}</td>
                                            <td class="p-2 text-right w-1/6">Rp
                                                {{ number_format($r->anggaran, 2, ',', '.') }}
                                            </td>
                                            <td class="p-2 text-right w-1/6">Rp
                                ...
                                <x-slot name="footer">
                                    ...
                                    @endphp
                                    <tr class="font-semibold border-t">
                                        <td class="p-2 text-left">Total Belanja {{ $b->nama }}</td>
                                        <td class="p-2 text-right">Rp {{ number_format($totalAnggaran, 2, ',', '.') }}
                                        </td>
                                        <td class="p-2 text-right">Rp {{ number_format($totalRealisasi, 2, ',', '.') }}
                                        </td>
                                        <td class="p-2 text-right">Rp {{ number_format($totalSelisih, 2, ',', '.') }}
                                        </td>
                                    </tr>
                                </x-slot>
            ...
            <div class="bg-white rounded-xl p-4 mb-4 shadow-lg">
                ...
                <div class="space-y-4">
                    ...
                    <x-table>
                        <x-slot name="head">
                            <tr>
                                <th class="p-2 text-left w-1/2">PENERIMAAN PEMBIAYAAN</th>
                                <th class="p-2 text-right w-1/6">Anggaran</th>
                                <th class="p-2 text-right w-1/6">Realisasi</th>
                                <th class="p-2 text-right w-1/6">Selisih</th>
                            </tr>
                    ...
                    <x-table>
                        <x-slot name="head">
                            <tr>
                                <th class="p-2 text-left w-1/2">PENGELUARAN PEMBIAYAAN</th>
                                <th class="p-2 text-right w-1/6">Anggaran</th>
                                <th class="p-2 text-right w-1/6">Realisasi</th>
                                <th class="p-2 text-right w-1/6">Selisih</th>
                            </tr>
                    ...
                    <x-table>
                        <x-slot name="head">
                            <tr>
                                <th class="p-2 text-left w-1/2">PEMBIAYAAN NETTO</th>
                                <th class="p-2 text-right w-1/6">Anggaran</th>
                                <th class="p-2 text-right w-1/6">Realisasi</th>
                                <th class="p-2 text-right w-1/6">Selisih</th>
                            </tr>
